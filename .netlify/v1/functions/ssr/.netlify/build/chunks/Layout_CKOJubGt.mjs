import { c as createComponent, r as renderTemplate, m as maybeRenderHead, s as spreadAttributes, f as renderSlot, d as renderComponent, e as createAstro, a as addAttribute, F as Fragment, u as unescapeHTML, b as renderHead } from './astro/server_Ci3rEjlL.mjs';
import 'kleur/colors';
...
import { $ as $$Image } from './_astro_assets_CwgT6DvA.mjs';
import { s as sanityClient } from './page-ssr_EDlWylL8.mjs';
...
async function getContentBySlug(slug, language) {
...
const $$Link = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Link;
  const { data, inverse, style } = Astro2.props;
  let href;
  ...
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(`${style} ${inverse ? "-inverse" : ""}`, "class")}${addAttribute(data.link.blank ? "_blank" : "", "target")}${addAttribute(href, "href")}>${data.link.text} </a>`;
...
const $$TextAndImage = createComponent(($$result, $$props, $$slots) => {
  ...
  if (block.fullWidth) {
    ...
    imgContainer = "flex flex-col gap-6 md:gap-16 xl:gap-32 md:items-start xl:items-center";
    imageAlignment = imgPosition === "image-right" ? "md:flex-row-reverse" : "md:flex-row";
  }
  return renderTemplate`${maybeRenderHead()}<section${addAttribute(`${imgContainer} ${imageAlignment}`, "class")}> ${hasValidImage && renderTemplate`${renderComponent($$result, "Image", $$Image, { "sizes": halfWidthSizes(), "srcset": srcSet(imageUrl ?? ""), "width": 1e3, "height": 1e3, "src": imageUrl ?? "", "alt": block.image?.alt ?? "", "class": `${block.fullWidth ? "aspect-square" : "img-rounded aspect-[7/6] md:w-1/2"} object-cover` })}`} <div${addAttribute(`spacing-y ${block.fullWidth ? "px-4 py-6 lg:py-12 xl:py-20" : "md:w-1/2"}`, "class")}> <div class="block-heading"> <span>${block.tagline}</span> <h2>${block.title}</h2> </div> <div class="rich-text"> ${renderComponent($$result, "PortableText", $$PortableText, { "value": block.richText })} </div> <div class="flex flex-wrap gap-3"> ${block.links?.map((link, index) => renderTemplate`${renderComponent($$result, "Link", $$Link, { "data": link, "style": `${index === 0 ? "btn btn--primary" : "btn btn--outline"}` })}`)} </div> </div> </section>`;
...
const $$ImageGallery = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="w-full"> <div${addAttribute(`gap-6 grid grid-cols-1 xl:gap-10 ${grid}`, "class")}> ${block.images.map((image, index) => {
    ...
    return renderTemplate`${renderComponent($$result, "Image", $$Image, { "sizes": fullWidthSizes(), "srcset": srcSet(imageUrl), "width": 1e3, "height": 1e3, "src": imageUrl, "alt": image.alt ?? "", "class": `object-cover w-full h-full  ${isFirst ? "img-rounded-first" : ""} ${isLast ? "img-rounded-last" : ""} ${aspectRatio}` })}`;
  })} </div> </div>`;
...
const $$PageBuilder = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div> ${blocks.map((block) => {
    ...
    return Component ? renderTemplate`<section${addAttribute(`${block.fullWidth ? "" : "section"} `, "class")}> <div${addAttribute(` ${block.fullWidth ? "!p-0" : "container"}`, "class")}> ${renderComponent($$result, "Component", Component, { "block": block })} </div> </section>` : null;
  })} </div>`;
...
const $$VisualEditing = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${enabled ? renderTemplate`${renderComponent($$result, "VisualEditingComponent", null, { "client:only": "react", "zIndex": zIndex, "client:component-hydration": "only", "client:component-path": "/Users/arnaud/Documents/github/silsomzorg-sanity/node_modules/@sanity/astro/dist/visual-editing/visual-editing-component", "client:component-export": "VisualEditingComponent" })}` : null}`;
}, "/Users/arnaud/Documents/github/silsomzorg-sanity/node_modules/@sanity/astro/dist/visual-editing/visual-editing.astro", void 0);
